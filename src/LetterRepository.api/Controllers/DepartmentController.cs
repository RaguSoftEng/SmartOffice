...
namespace LetterRepository.api.Controllers
{
    ...
    public class DepartmentController : ControllerBase
    {
        ...
        [HttpPost("purpose/{id}")]
        public async Task<dynamic> Post([FromBody] DepartmentPurpose obj, long id)
        {
            return await _departmentRepository.UpdatePupose(id, obj);
        }

        [HttpGet("purpose/{depid}")]
        public async Task<List<DepartmentPurpose>> GetAllPurpose(long depid)
        {
            return await _departmentRepository.GetAllPurpose(depid);
        }

        [HttpGet("purposeList/{depid}")]
        public async Task<dynamic> GetPurposeAsListValue(long depid)
        {
            return await _departmentRepository.GetPurposeAsListValue(depid);
        }
    }
}
